...
(function() {
	$(window).on('load', function(){
		...
		// Initially hide items after first 9
		var allItems = grid.find('.works__item');
		...
		$('.works__more').on('click', function() {
			var hiddenItems = grid.find('.works__item:hidden');
			var nextItems = hiddenItems.slice(0, itemsPerPage);
		...
		// Rest of the existing isotope code
...
(function() {
	...
	$(window).on('load', function() {
		...
		if (footerHeight <= $(window).height()) {
			...
			footer.css({ 'position': 'relative', 'left': leftValue, 'right': '0', 'bottom': '0'});
...
(function() {
	$(document).ready(function() {
		var loadMoreBtn = $('.works__more');
		var worksList = $('.__js_works-filter');
		var items = worksList.find('.works__item');
		var itemsPerPage = 9;
		var visibleItems = itemsPerPage;
		
		// Initially hide all items except first 9
		...
		loadMoreBtn.on('click', function(e) {
			...
			items.slice(visibleItems, visibleItems + itemsPerPage).fadeIn(500);
